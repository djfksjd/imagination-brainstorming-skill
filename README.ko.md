...
- **금지된 형용사가 평범한 어휘일 수 있다.** 소설에서 *magical*이나 *delightful*은 주장이 아니라 지칭이고, 실제로 *"이 지역은 magical하지 않다"*가 그 말을 했다는 이유로 거부된 적이 있다. 그 구간을 자리에서 표시하면 된다 — `<!-- mention: hollow-magical -->the region is not magical<!-- /mention -->` — 해제는 그 구간과 그 규칙에만 적용된다. 그 낱말이 정말 사용이 아니라 언급인지까지는 확인할 수 없다. 다만 규칙 전체를 통째로 푸는 대신, 주장을 겉으로 드러내고 검토 가능하게 만든다. 표시에는 상한이 있다. 표시 하나에 규칙 id 하나, 구간은 한 문단(200 단위)까지, 그 안에 부정이나 큰따옴표로 묶은 낱말이 있어야 하고, 문서 전체에서 다섯 개까지다. 그리고 첫 직감과 사용자 본인의 배제 항목은 이 경로로도, 다른 어떤 경로로도 해제되지 않는다. 예전에는 모든 id를 나열한 표시 하나가 계약 전체를 해제했다.
