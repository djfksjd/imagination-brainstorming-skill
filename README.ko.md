...
**`approaches.json`은 직접 쓰는 파일이다.** 프레임은 `deal.py`가 뽑아 주고, 뽑힌 프레임마다 하나씩 접근안을 써서 `approaches` 키를 가진 객체에 담는다 — `id`, 프레임 덱에서 가져온 `frame_id`, 86단위 이상의 `summary`, 이 브리프에서 *이* 안이 어떻게 무너지는지 말하는 43단위 이상의 `failure_mode`, 그 프레임이 요구하는 자리를 이 브리프에서 무엇이 채우는지 밝히는 36단위 이상의 `frame_fit`, 그리고 정확히 하나에만 참으로 두는 `unsafe_seat`. 모든 필드와 하한선은 `references/decks/approaches-schema.json`에 적혀 있고, `references/example-approaches.json`은 형태를 그대로 베껴 갈 수 있는 통과 예시다.

이 하한선들은 글자 수가 아니라 단위로 세며, **추정이 아니라 실측으로** 정해졌다. 받아들일 만한 가장 얇은 첫 접촉 장면 하나를 라틴·한국어·일본어·중국어·태국어·데바나가리·히브리어·아랍어로 옮겨, 실제로 돌아가는 함수로 재고, 같은 소재를 장면이 아니라 서술로 쓴 글도 함께 쟀다. 장면은 186~283단위, 서술은 41~64단위로 나왔다. 그래서 하나의 숫자가 여덟 언어 모두에서 둘을 가른다. 하한선은 가장 밀도가 높은 문자에서 가장 얇은 정당한 장면까지 통과시키는 가장 높은 값이다. 하나의 숫자로 할 수 없는 일은 모든 문자에 똑같이 엄격해지는 것이다. 같은 내용이 중국어에서는 라틴 문자보다 약 3분의 1 적은 단위로 측정되므로, 이 기준은 라틴 문자 산문에 더 빡빡하다. 그렇게 둔 이유는 남의 언어로 쓴 정당한 글을 거부하는 쪽이 더 나쁜 실패이기 때문이다.
...
>
> 손으로 편집한 금지 목록은 자기만의 `structural_patterns[].regex`를 추가할 수 있는데, `(a+)+` 같은 모양의 패턴은 어떤 입력에서 파이썬 매칭 엔진이 지수적으로 오래 걸리게 만든다 — 결코 응답하지 않는 게이트는, 기다리는 쪽 입장에서는 통과한 게이트와 다를 바 없다. 이제 이런 패턴은 실행되기 전에, id를 이름으로 명시하며 거부된다. 이 검사는 고전적인 중첩 반복 모양을 찾는 결정적 스캔이므로 이 위험을 좁힐 뿐 없애지는 못한다 — 인식하지 못하는 파국적 모양은 여전히 느릴 수 있다 — 그리고 macOS와 Linux에서는 스캔이 놓친 것을 잡는 시간제한 안전장치가 하나 더 있는데, 이 안전장치는 Windows에서는 쓸 수 없다. 긴 줄은 4000자에서 잘리는 대신 4000자짜리 창을 겹쳐가며 전부 훑는다 — 첫 판본은 잘라냈고, 그래서 구조 금지 규칙이 그 지점 이후로 조용히 발동을 멈췄다. 그것은 대체하려던 멈춤보다 나쁘다. 줄 단위 시간 예산이 떨어지면 패턴 id를 밝히며 거부한다. 겹치는 512자보다 긴 일치가 창 경계에 걸치면 여전히 놓칠 수 있다.
>
> **`imagination-engine`은 이 지점을 반대로 결정했고, 그렇다고 명시한다.** 그쪽 게이트는 사용자가 넣은 `structural_patterns[].regex`를 아예 컴파일하기를 거부하고, 실행하는 대신 패턴을 이름으로 지목하며 거부를 출력한다 — 그 게이트는 이미 실행 시점에 공급되는 다른 모든 정책을 거부하고 있고(`--rubric`도, `--min-mean`도, 존중되는 `--allow`도 거기엔 없다), 타이머 기반 방어는 실제로 무엇이 검사됐는지가 호스트 머신의 속도에 좌우된다는 것 — 기계마다 달라지는 판정이라는 것 — 이 이유다. 이 게이트는 거부 대신 그 대가를 감수한다. 위의 구조 스캔과 문자 상한은 두 경우 모두 유효하지만, 스캔이 놓친 것을 잡는 `SIGALRM` 타이머는 POSIX 전용이라 Windows에서는 스캔과 문자 상한만이 게이트를 지키고, macOS나 Linux에서도 느린 기계에서는 멈추고 빠른 기계에서는 통과하는 패턴은 engine의 거부가 피하려는 바로 그 기계 의존적 판정이다. 두 스킬 사이를 오간다면, 여기서는 조용히 컴파일되는 패턴이 저쪽에서는 이름으로 거부될 것을 예상해야 한다.
...
- **금지된 형용사가 평범한 어휘일 수 있다.** 소설에서 *magical*이나 *delightful*은 주장이 아니라 지칭이고, 실제로 *"이 지역은 magical하지 않다"*가 그 말을 했다는 이유로 거부된 적이 있다. 그 구간을 자리에서 표시하면 된다 — `<!-- mention: hollow-magical -->the region is not magical<!-- /mention -->` — 해제는 그 구간과 그 규칙에만 적용된다. 그 낱말이 정말 사용이 아니라 언급인지까지는 확인할 수 없다. 다만 규칙 전체를 통째로 푸는 대신, 주장을 겉으로 드러내고 검토 가능하게 만든다. 표시에는 상한이 있다. 표시 하나에 규칙 id 하나, 구간은 한 문단(200 단위)까지, 그 안에 부정이나 큰따옴표로 묶은 낱말이 있어야 하고, 문서 전체에서 다섯 개까지다. 그리고 첫 직감과 사용자 본인의 배제 항목은 이 경로로도, 다른 어떤 경로로도 해제되지 않는다. 예전에는 모든 id를 나열한 표시 하나가 계약 전체를 해제했다. 해제는 표시된 문자에만 정확히 적용된다. 표시된 구간은 그 자리에서 비워져 따로 검사되므로, 문서 다른 곳의 동일한 문장은 다른 구간이고 그대로 걸린다 — 예전에는 표시 하나가 파일 안의 동일한 모든 줄을 해제했다. 표시는 낱말 경계에 놓여야 한다.
